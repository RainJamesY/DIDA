...
class DAFormerHead(BaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        os_size = x[0].size()[2:]
        _c = {}
        for i in self.in_index: # in_index = 4; 4 feature maps in x
            ...
            if _c[i].size()[2:] != os_size: # bilinearly upsample the features to the size of F1
        ...
        # pdb.set_trace()
        # F_cat = torch.cat(list(_c.values()))

        Fuse_feat  = self.fuse_layer(torch.cat(list(_c.values()), dim=1)) # feature fusion [2, 256, 128, 128]
        x = self.cls_seg(Fuse_feat) # classify each pixel [2, 19, 128, 128]

        return Fuse_feat, x 
    
    # orginal decode function
    def forward_old(self, inputs):
        ...
        x = self.fuse_layer(torch.cat(list(_c.values()), dim=1)) # feature fusion [2, 256, 128, 128]
        x = self.cls_seg(x) # classify each pixel [2, 19, 128, 128]

        return x    
    
    # Add decode_features function to decode multilevel features x
    def decode_features(self, inputs):
        ...
        # pdb.set_trace()
        F_cat = torch.cat(list(_c.values()), dim=1)
        Fuse_feat = self.fuse_layer(F_cat)      # feature fusion
